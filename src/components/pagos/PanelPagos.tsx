...
import { getCopmContractAddress, getCopmTestnetAddress, getPlatformWalletAddressPublic, parseTokenAmount } from '@/config/celo';
...
export default function PanelPagos() {
  ...
  const handleMetaMaskPayment = useCallback(async (cuota: EnrichedCuota) => {
    ...
    try {
      ...
      // CRITICAL: monto_cuota is in COPm, must convert to wei (10^18)
            ...
            <div className="px-6 py-4 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-slate-200">
                  {formatCop(first.credito_monto)} COPm
                </p>
